...
v2=voom(dge_cc,design=design1,plot=TRUE)    #iPSC vs other stages
...
for (i in colnames(d[2:9])) {
 ...
 DE_stages<-list()  
   ...
   set.caption("Top DE genes for iPSC")
   pandoc.table(head(sig_iPSC_stage),split.table = Inf)
   set.caption("Top DE genes for DE")
   pandoc.table(head(sig_DE_stage),split.table = Inf)
   set.caption("Top DE genes for PGT")
   pandoc.table(head(sig_PGT_stage),split.table = Inf)
   set.caption("Top DE genesfor PFG")
   pandoc.table(head(sig_PFG_stage),split.table = Inf)
   set.caption("Top DE genes for PE")
   pandoc.table(head(sig_PE_stage),split.table = Inf)
   set.caption("Top DE genes for EP")
   pandoc.table(head(sig_EP_stage),split.table = Inf)
   set.caption("Top DE genes for EN6")
   pandoc.table(head(sig_EN6_stage),split.table = Inf)
   set.caption("Top DE genes for EN7")
   pandoc.table(head(sig_EN7_stage),split.table = Inf)
...
### Longitudinal plots of top DE genes for each stage
  ...
  stages_longplot=c("iPSC", "DE", "PGT", "PFG", "PE", "EP","ENstage6", "ENstage7")
   ...
   make_long_plots=function(y,.tpm=tpm,.samples=samples,n=9,name){
    ...
    ggsave(paste("/Users/Marta/Documents/WTCHG/DPhil/Plots/conservative_counts/Diff_v2_top9genes_nocontrasts",name,currentDate,".jpg",sep="_"),p,width=8,height=8,units="in",dpi=300)
     
     return(p)
   }  # function for printing the long plots

 for(i in names(DE_stages)){   # loop trhough list of dataframes to plot
   print(i)
   p= make_long_plots(DE_stages[[i]],name=i)
   print(p)
 }
   ...
   # save results as they are:
   # 
   # for (i in names(DE_stages)){
   #   write.csv(DE_stages[i],quote=F,row.names=F,file=paste("/Users/Marta/Documents/WTCHG/DPhil/Data/Results/Diff_v2/Voom/conservative_counts/",currentDate,"_sig_maxvals_",i,"_diff_expression_results.csv",sep=""))
   #  
   # }
   
   
   # for adj p-vals<0.01 and logFC>1 (keep in mind this is not how they do it in limma package manual!):
   ...
   for (i in names(DE_stages_logFC1)){
     write.csv(DE_stages_logFC1[i],quote=F,row.names=F,file=paste("/Users/Marta/Documents/WTCHG/DPhil/Data/Results/Diff_v2/Voom/conservative_counts/",currentDate,"_sig_maxvals_",i,"_diff_expression_results_logFC1.csv",sep=""))
